-- TABLE -- 
  CREATE TABLE `banco`.`events_count` (
...
USE altubot_x;
DELIMITER |
CREATE TRIGGER tg_x_events_in AFTER INSERT ON events
FOR EACH ROW
BEGIN
	IF ((NEW.assistant_id = '1') 
		AND (NEW.event_name IN ('horario_atendimento','pesquisa_satisfacao'))
		AND ((SELECT EXISTS(SELECT * FROM banco.events_count
		...
		THEN INSERT INTO banco.events_count
...
-- SELECT -- 
INSERT INTO banco.events_count (`date`, `assistant_id`, `extra2`, `transbordo`, `promoters`, `passives`, `detractors`, `totalnota`)
